...
transLocAPI = function(){
    HTTP.call("GET", url, {
        ...
        if(!err) {
            ...
            result.data[first].forEach( function(element, index) {
                ...
                // console.log(element)
                var response = [];
                var all = getSegment(element, function(response) {
                    var array = Object.keys(response).map(function (key) {return response[key]});
                    console.log(array);
                    element["segments"] = array;
                    Shuttles.insert(element);
                });
            });
        } else {
            console.log( err );
    ...
    getSegment = function(element, fn) {
